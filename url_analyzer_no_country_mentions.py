...
KNOWN_US_DOMAINS = {
    ...
    'arxiv.org',  # arXiv - Cornell University academic preprint repository
}
...
def extract_addresses_from_text(page_text):
    ...
    for i, line in enumerate(lines):
        ...
        # PRIMARY: zip/postal code AND city/state/country (most reliable)
        ...
        # SECONDARY: For countries without standard postal systems (Pakistan, Indonesia, Hong Kong, UAE)
        # Allow street indicators + city + country, but ONLY within tight 80-char window
        elif not has_zip and has_location and len(line_clean) <= 80:
            # Only for specific countries that often don't use postal codes
            countries_without_standard_postal = ['Pakistan', 'Indonesia', 'UAE', 'Hong Kong']

            if location_country_name in countries_without_standard_postal:
                # Check for street indicators within the same line
...
def analyze_page_content(soup, url):
    ...
    # 1. Collect information from main page
    # Determine if this is an info page (about/contact/terms/legal) or regular page
    url_lower = url.lower()
    is_info_page = any(keyword in url_lower for keyword in ['about', 'contact', 'terms', 'privacy', 'legal'])

    if is_info_page:
        # Info pages: search full page for addresses
        page_text = soup.get_text()
    else:
        # Regular pages: search only footer for addresses
        footer = soup.find('footer')
        if footer:
            page_text = footer.get_text()
        else:
            # Fallback: try to find footer by common class/id names
            ...
            if footer_divs:
                ...
                # No footer found, use last 20% of page as approximation
                full_text = soup.get_text()
                text_lines = full_text.split('\n')
                footer_start = int(len(text_lines) * 0.8)
                page_text = '\n'.join(text_lines[footer_start:])

    # Look for physical addresses in selected text (footer or full page)
