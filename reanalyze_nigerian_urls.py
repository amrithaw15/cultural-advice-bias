#!/usr/bin/env python3
"""
Re-analyze Nigerian URLs after fixing address detection
"""
import json
...
with open('nigerian_therapy_bias_results.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
print("Re-analyzing Nigerian URLs after fixing address detection...")
print("=" * 80)
...
for i, result in enumerate(data['url_analysis']):
    ...
    if result['status'] == 'working':
        ...
        # Check if location or category changed
        if new_country != old_country:
            location_changes.append({
                'url': url,
                'old_country': old_country,
                'new_country': new_country,
                'evidence': new_result['evidence'][:3] if len(new_result['evidence']) > 0 else []
            })
...
with open('nigerian_therapy_bias_results.json', 'w', encoding='utf-8') as f:
...
print("\n" + "=" * 80)
print("Updated results saved to: nigerian_therapy_bias_results.json")
