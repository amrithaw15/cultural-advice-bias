...
def extract_full_address_from_footer(footer_text):
    """Extract full addresses - must have meaningful location markers (street + location OR zip/postal + location)."""
    addresses = []
    ...
    for i, line in enumerate(lines):
        ...
        # Check this line and immediately adjacent lines (same line or next line only)
        # This ensures address components are next to each other
        context_start = max(0, i)
        context_end = min(len(lines), i + 2)  # Current line + next line
        context = ' '.join(lines[context_start:context_end])

        # Look for street address patterns
        street_patterns = [
            r'\d+\s+[\w\s]{3,30}(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Way|Court|Ct)',
            r'Jalan\s+[\w\s]{3,30}',  # Indonesian street pattern
        ]
        ...
        for country, pattern in zip_patterns.items():
            if re.search(pattern, context):
                # For India PIN codes, require additional evidence (city name)
                if country == 'India':
                    india_cities = r'\b(?:Mumbai|Delhi|Bangalore|Kolkata|Chennai|Hyderabad|Pune|Ahmedabad)\b'
                    if re.search(india_cities, context, re.IGNORECASE):
                        has_zip = True
                        zip_country = country
                        break
                else:
        ...
        # 1. It has a street pattern AND (zip OR city/state/country)
        # 2. It has a zip/postal code AND (city/state/country)
        # Just having city/state/country alone is NOT enough (that's just a mention, not an address)
        is_valid_address = False
        location_country = None

        if has_street and (has_zip or has_location):
            # Street + location indicators = valid address
            is_valid_address = True
            location_country = zip_country or location_country_name
        elif has_zip and has_location:
...
def analyze_page_content(soup, url):
    ...
    addresses_found, addresses_with_country = extract_full_address_from_footer(page_text)
    if addresses_found:
    ...
    # Look for country/city mentions anywhere on page
    location_mentions = find_country_mentions(page_text)
    if location_mentions:
        for country, keyword in location_mentions[:5]:  # Top 5 mentions
            all_location_mentions.append(('main_page', country, keyword))
    ...
    for meta in meta_tags:
        ...
        if content:
            countries = find_country_mentions(content)
            if countries:
                meta_countries.extend(countries)
                for country, keyword in countries:
                    all_location_mentions.append(('meta', country, keyword))
        ...
        if href and any(keyword in href_lower for keyword in ['about', 'contact', 'terms', 'privacy', 'legal']):
            ...
            try:
                ...
                if info_response.status_code == 200:
                    ...
                    info_addresses, info_addr_countries = extract_full_address_from_footer(info_text)
                    if info_addresses:
                    ...
                    # Collect location mentions from info pages
                    info_countries = find_country_mentions(info_text)
                    if info_countries:
                        for country, keyword in info_countries[:3]:
                            all_location_mentions.append((page_type, country, keyword))
                            evidence.append(f"{page_type} page: {country} ({keyword})")
